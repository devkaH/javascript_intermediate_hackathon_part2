...
/*  Seek and Destroy
	...
	*/
	function destroyer(arr) {
		...
		return arr.filter(function(elt) { return args.indexOf(elt) == -1 ;
...
/*  Where do I belong
	...
	*/
	...
	function getIndexToIns(arr, num) {
		...
		let indexNum = sortedArr.findIndex(function(elt){
			return num <= elt ;
		});
...
/*  Sum All Numbers in a Range
	...
	*/
	function sumAll(arr) {
		...
		console.log(list);
		let sum = list.reduce(function(sum, current) {
			return sum + current;
		}, 0);
...
/*  Diff Two Arrays 
	...
	*/
	function diffArray(arrA, arrB) {
		...
		diffArr = arrA.filter(function(val) {
			return arrB.indexOf(val) === -1;
		});

		diffArr = diffArr.concat(arrB.filter(function(val) {
			return arrA.indexOf(val) === -1;
		}));
...
/*  Confirm the Ending
	...
	*/
	function confirmEnding(str, target) {
		if (str.indexOf(' ') != -1) {
			let end = str.substr(str.lastIndexOf(' ') + 1, target.length + 1);
			return end === target ;
		} else {
	...
	test(confirmEnding('Walking on water and developing software from a specification are easy if both are frozen', 'specification'), false);
...
/*  Chunky Monkey
	...
	*/
	function chunkArrayInGroups(arr, size) {
		var groups = arr.map( function(elt,i){ 
...
/*  Mutations
	...
	*/
	function inclus(elt,firstElt){
		
		return firstElt.indexOf(elt) >= 0;
	}
	function mutation(arr) {
		var firstElt = arr[0].toLowerCase().split('');

		var secondElt = arr[1].toLowerCase().split('');

		return  secondElt.every(function(elt){
			return inclus(elt,firstElt);
		});
